...
def get_roi_mask(img_bgr: np.ndarray):
    ...
    if not contours:
        empty = np.zeros(img_bgr.shape[:2], np.uint8)
    ...
    h, w   = img_bgr.shape[:2]
    offset = int(CFG["roi_offset"])

    # mask_orig = fill ben trong contour xanh
    mask_orig = np.zeros((h, w), np.uint8)
    cv2.fillPoly(mask_orig, [c_orig], 255)

    # Vung NGOAI mask_orig
    outside = cv2.bitwise_not(mask_orig)
    ...
    # between = ngoai contour xanh
    # va khoang cach <= offset
    between = np.zeros((h, w), np.uint8)
    between[(outside > 0) & (dist <= offset)] = 255

    if np.count_nonzero(between) == 0:
        between = outside.copy()

    # c_dila = contour ngoai cung
    mask_full = cv2.bitwise_or(mask_orig, between)
    cdil, _ = cv2.findContours(
        mask_full, cv2.RETR_EXTERNAL,
        cv2.CHAIN_APPROX_SIMPLE)
    if not cdil:
...
def get_roi_mask_local(img_bgr: np.ndarray, cfg: dict):
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    ...
    contours = [c for c in contours
                if cv2.contourArea(c) >= cfg["roi_min_area"]]
    if not contours:
        empty = np.zeros(img_bgr.shape[:2], np.uint8)
        return empty, None, None
...
# ════════════════════════════════════════════════════════════════
# ════════════════════════════════════════════════════════════════
# WORKER & BATCH - FIX RACE CONDITION
# ════════════════════════════════════════════════════════════════
def process_one(args):
    ...
    # ── FIX: Snapshot CFG một lần, dùng cho toàn bộ ảnh ──────
    # Tránh CFG bị thay đổi từ GUI thread giữa chừng
    cfg_snap = dict(CFG)  # shallow copy đủ dùng

    # ── FIX: Build kernel LOCAL cho thread này ────────────────
    # Không dùng global KERNEL, K_SEP, K1, K_GREEN
    local_kernel = _make_kernel(
        cfg_snap["morph_k"],
        cfg_snap["morph_shape"])
    local_k_sep  = _make_kernel(
        cfg_snap["sep_open_k"],
        cfg_snap["morph_shape"])
    local_k1     = _make_kernel(
        cfg_snap["ring_dilate_k"],
        cfg_snap["morph_shape"])
    local_k_green = _make_kernel(
        cfg_snap["green_dilate_k"],
        cfg_snap["morph_shape"])

    # ── FIX: Build CLAHE LOCAL ────────────────────────────────
    local_clahe = None
    if cfg_snap.get("clahe_enable", True):
        # FIX: Ho tro ca 2 key format
        # main.py gui "clahe_grid" (tuple)
        # inspector.py dung "clahe_grid_x/y"
        _grid   = cfg_snap.get("clahe_grid", (14, 14))
        grid_x  = cfg_snap.get("clahe_grid_x", _grid[0])
        grid_y  = cfg_snap.get("clahe_grid_y", _grid[1])
        local_clahe = cv2.createCLAHE(
            clipLimit    = cfg_snap["clahe_clip"],
            tileGridSize = (grid_x, grid_y))

    # ── Pipeline với local objects ────────────────────────────
    img_crop, gray, grad, low_mask, cleaned = \
        _pipeline_cpu_local(img, cfg_snap, local_clahe,
                            local_kernel)

    roi_mask, c_orig, c_dila = \
        get_roi_mask_local(img_crop, cfg_snap)

    label, ratio, noise_info = \
        classify_local(cleaned, roi_mask, cfg_snap,
                       local_k_sep, local_k1, local_k_green)

    out_path = (out_ok if label == "OK" else out_ng) / \
               f"{img_path.stem}_{label}_{ratio*100:.0f}pct.jpg"

    save_result_local(out_path, label, ratio, noise_info,
                      img_crop, gray, grad, low_mask, cleaned,
                      roi_mask, c_orig, c_dila,
...
# ════════════════════════════════════════════════════════════════
# LOCAL PIPELINE - dùng cfg_snap và local objects
# ════════════════════════════════════════════════════════════════
def _pipeline_cpu_local(img: np.ndarray,
                        cfg:        dict,
                        local_clahe,
                        local_kernel: np.ndarray):
    """Pipeline CPU với local objects - thread safe"""
    order = cfg["preprocess_order"]
    k     = cfg["blur_ksize"]
    sg    = cfg["blur_sigma"]
    ...
    # Gray + CLAHE local
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    if cfg["clahe_enable"] and local_clahe is not None:
        gray = local_clahe.apply(gray)

    # Gradient
    grad, edge = _compute_gradient_cpu_local(gray, cfg)

    # Threshold
    low_mask = edge if edge is not None \
               else _apply_threshold_cpu_local(grad, cfg)

    # Morphology local kernel
    cleaned = cv2.morphologyEx(
        low_mask, cv2.MORPH_OPEN, local_kernel,
        iterations=cfg["morph_open_iter"])
    cleaned = cv2.morphologyEx(
        cleaned, cv2.MORPH_CLOSE, local_kernel,
        iterations=cfg["morph_close_iter"])
    ...
    if cfg["morph_extra_enable"]:
        ...
        op      = ops.get(cfg["morph_extra_type"],
                          cv2.MORPH_DILATE)
        cleaned = cv2.morphologyEx(
            cleaned, op, local_kernel,
            iterations=cfg["morph_extra_iter"])

    return img, gray, grad, low_mask, cleaned


def _compute_gradient_cpu_local(gray: np.ndarray,
                                 cfg:  dict):
    """Gradient với cfg local"""
    method = cfg["grad_method"]
    try:
        if method == "scharr":
            gx   = cv2.Scharr(gray, cv2.CV_32F, 1, 0)
        ...
        cv2.normalize(grad, grad_norm, 0, 255,
                      cv2.NORM_MINMAX)
        return grad_norm.astype(np.uint8), None

    except Exception as e:
        print(f"[WARN] _compute_gradient_cpu_local: {e}")
        return np.zeros_like(gray, dtype=np.uint8), None


def _apply_threshold_cpu_local(grad: np.ndarray,
                                cfg:  dict):
    """Threshold với cfg local"""
    t = cfg["grad_thresh_type"]
...
def get_roi_mask_local(img_bgr: np.ndarray, cfg: dict):
    ...
    # ════════════════════════════════════════════════
    # FIX CHINH XAC:
    # Dung distance transform tu NGOAI mask_orig
    # → moi pixel ngoai contour xanh
    #   co gia tri = khoang cach den contour xanh
    # → lay vung co khoang cach <= offset
    # → day la vung "cach deu" theo moi huong
    # ════════════════════════════════════════════════

    # Vung NGOAI mask_orig = 255 (de tinh dist)
    # Vung TRONG mask_orig = 0
    outside = cv2.bitwise_not(mask_orig)

    # Distance transform: moi pixel ngoai
    # co gia tri = khoang cach den bien xanh
    dist = cv2.distanceTransform(
        outside,
        cv2.DIST_L2,
        cv2.DIST_MASK_PRECISE)

    # Vung between = ngoai contour xanh
    # va khoang cach <= offset
    between = np.zeros((h, w), np.uint8)
    between[(outside > 0) & (dist <= offset)] = 255
...
# LOCAL CLASSIFY
# ════════════════════════════════════════════════════════════════
def classify_local(cleaned:      np.ndarray,
                   roi_mask:     np.ndarray,
                   cfg:          dict,
                   local_k_sep:  np.ndarray,
                   local_k1:     np.ndarray,
                   local_k_green: np.ndarray):
    """Classify với local kernels - thread safe"""
    px    = (roi_mask > 0) if roi_mask is not None \
            else np.ones_like(cleaned, bool)
    total = int(px.sum())
    _e    = {"cond": "A", "red_count": 0,
             "red_max_area": 0,
             "details": [], "largest_green_px": 0}
    if total == 0:
        return "OK", 1.0, _e
    ...
    # Dùng local_k_sep thay K_SEP global
    high_sep = cv2.morphologyEx(
        high_roi, cv2.MORPH_OPEN, local_k_sep)
    n_red, red_lbl, red_stats, _ = \
    ...
    # Dùng local_k_green thay K_GREEN global
    green_dil  = cv2.dilate(green_roi, local_k_green)
    all_pass   = True
    details    = []
    max_defect = cfg["max_defect_count"]
    ...
    for i in range(1, n_red):
        ...
        dot  = (red_lbl == i).astype(np.uint8) * 255
        # Dùng local_k1 thay K1 global
        ring = cv2.bitwise_xor(
            cv2.dilate(dot, local_k1), dot)
        fi  = not np.any(
            (ring > 0) & (green_dil == 0))
        or_ = roi_mask is not None and \
              np.any((ring > 0) & (roi_mask == 0))
...
# LOCAL SAVE RESULT
# ════════════════════════════════════════════════════════════════
def save_result_local(out_path, label, ratio, noise_info,
                      img_crop, gray, grad, low_mask, cleaned,
                      roi_mask, c_orig, c_dila,
                      cfg: dict,
                      local_k_sep: np.ndarray):
    """save_result với local objects - thread safe"""
    h, w = gray.shape
    clr  = (0, 255, 0) if label == "OK" else (0, 0, 255)
    B    = lambda g: cv2.cvtColor(g, cv2.COLOR_GRAY2BGR)
    RL   = lambda p: draw_roi_lines(
    ...
    all_panels = {
        ...
        3: RL(B(low_mask)),
        4: RL(B(cleaned)),
        5: RL(B(cv2.bitwise_and(gray, gray,
                                 mask=cleaned))),
        6: None,
    }
    ...
    nf  = sum(1 for d in details if not d["pass"])
    lns = [
        (f"[{label}] cond=[{noise_info.get('cond','?')}]",
         clr),
        (f"[A] low={ratio*100:.1f}%>="
         f"{cfg['ok_ratio']*100:.0f}%"
         f" {'✓' if ratio >= cfg['ok_ratio'] else '✗'}",
         (0, 255, 0) if ratio >= cfg["ok_ratio"]
         else (80, 80, 80)),
        (f"[B] green={noise_info.get('largest_green_px',0)}px"
         f" | dots:{len(details)} ✓{np_} ✗{nf}",
         (0, 255, 0) if nf == 0 else (0, 0, 255)),
    ...
    for d in ([x for x in details if not x["pass"]][:4] +
        ...
        lns.append((
            ...
            f"#{d['id']} {d['reason']}",
            (0, 220, 220) if d["pass"] else (0, 0, 255)))
    lns.append(("Yellow=ignored  Red=defect",
                (180, 180, 180)))
    for i, (txt, c) in enumerate(lns):
        cv2.putText(p6, txt, (8, 26 + i * 24),
